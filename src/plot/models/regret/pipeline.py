...
def build_v15_regret(games, *, raw_dir: str, trace: pl.DataFrame, completion: float = 0.80):
    ...
    games that actually loaded (so callers can report corpus size).
    """
    shots_parts, passup_parts, shotsel_parts = [], [], []
    opts_by_game, actions_by_game, states_by_game = {}, {}, {}
    for g in games:
    ...
    xmodel = train_xpoints(shots)

    for g in clean:
        pu = regret_from_states(states_by_game[g], actions_by_game[g], xmodel, trace, g)
        if pu.height:
            passup_parts.append(pu.with_columns(pl.lit("pass_up").alias("decision_kind")))
        ss = shot_selection_regret(opts_by_game[g], xmodel, g, completion=completion)
        if ss.height:
    ...
    regret = pl.concat([passup.select(V15_COLS), shotsel.select(V15_COLS)])
    return regret, shots, xmodel, clean
